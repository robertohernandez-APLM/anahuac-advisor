...
| 7 | **⚠️ FLAGEADO (2026-08-30) — `session_template.json` con vocabulario viejo** | La plantilla de sesión sigue en `professional_profile` / `current_seniority` / `functional_areas_current` / `hard_skills_strong`, mientras que `dnc_schema.json` y los tres scripts (`compute_match.py`, `validate_dnc.py`, `render_report.py`) ya usan `profile` / `seniority_level` / `current_functional_areas` / `current_hard_skills` (reconciliados 2026-07-24). Si el agente arma la sesión desde la plantilla, el motor no encuentra las áreas y `fa_score` cae a 0 — exactamente el bug que se corrigió en el código. **Pendiente:** alinear la plantilla al vocabulario del schema |
