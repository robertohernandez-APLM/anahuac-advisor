...
**Dashboard de leads** (`dashboard/index.html`, protegido por password) — **2 pestañas** desde el 2026-08-31: **Leads** y **Competencias y brechas** (esta última fusiona las antiguas pestañas «Radar de competencias» y «Reporte de brechas» en una sola página con **un solo juego de filtros** —programa · edad · industria · compañía— que alimenta sus 3 secciones: 1 Corte seleccionado · 2 Radar de competencias · 3 Brechas reales del funnel). **Exportación a PDF (2026-08-31):** botón «Exportar PDF» que usa `window.print()` + una hoja `@media print` (sin librerías: el usuario elige «Guardar como PDF» en el diálogo del navegador). Resuelve lo que rompía al imprimir: oculta navegación y filtros, fuerza `print-color-adjust:exact` (las barras y los radares SON el dato), **redibuja los radares a 560 px × escala 3** —el bitmap del canvas se dimensiona desde `clientWidth`, que cambia al imprimir—, quita el scroll horizontal de la tabla, repite su `thead` por página y evita filas partidas. Inyecta un encabezado *print-only* con el corte en texto, el conteo y la fecha (los `<select>` no se imprimen), y nombra el archivo desde `document.title`. Un check **«Anonimizar leads»** sustituye nombre y correo por un **folio estable** (`L-001`, posición en la hoja, no en el corte) en pantalla y en el PDF, para poder compartir el reporte con una empresa sin exponer datos personales:

1. **Leads** (pestaña propia) — tabla + filtros (fecha, industria, **compañía**, **rango de edad**, **inicio planeado**, búsqueda), modal de detalle y KPIs. La columna y el filtro **«Origen»** se **eliminaron** el 2026-08-29 (el modal de detalle sí lo sigue mostrando); en su lugar entró **«Inicio planeado»** (`planned_time`, con etiquetas humanizadas vía `PLANNED_TIME_LABELS`).
2. **Radar de competencias** (sección 2 de la página fusionada) — radares hard (8) + soft (10) dibujados en Canvas, **en vivo** desde los leads: la demanda de cada eje se infiere del mix de seniority × competencias prioritarias por nivel de la matriz. Filtros de **Programa recomendado + Rango de edad** (2026-08-07) **+ Industria + Compañía** (2026-08-29) que recalculan el corte.
3. **Corte seleccionado + Brechas reales** (secciones 1 y 3 de la página fusionada) — los mismos filtros (**Programa + Rango de edad + Industria + Compañía**) alimentan el panel **«Corte seleccionado»** con señales reales del funnel (nº de leads, `score-brechas` prom., match prom., distribución de `area-necesita`, mix de seniority). El 2026-08-29 la pestaña quedó **100% real**: la tabla demo «Brecha → Programa → Valor dual» (aspirantes ficticios) se reemplazó por **«Brechas reales del funnel»** — tabla por-lead poblada en `renderBrechas` con `key_gap` / `area-necesita` / `score-brechas` / fortalezas / `top_match_name`, filtrable, que solo lista leads con `key_gap`; y se eliminó la sección demo «Impacto 4 pilares». El puente Brecha→Programa→Valor dual **completo** (con brechas de competencia diagnosticadas) sigue viniendo de las sesiones del skill vía `gap_program_value_report.py`.
